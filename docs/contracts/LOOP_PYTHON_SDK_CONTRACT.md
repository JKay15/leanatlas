...
- boundary-equivalent `max_files_per_partition` values are accepted when the frozen helper-generated partition boundaries remain identical; authoritative replay freezes the emitted partition lineage, not an otherwise indistinguishable integer that yields the same boundaries.
...
- When `followup_partition_ids` is non-empty, replaying the helper-derived merged scope back into `build_pyramid_review_plan(...)` alongside the same `followup_partition_ids` MUST preserve the same provenance metadata and `strategy_fingerprint`; the empty-followup path still relies on the explicit `effective_scope_paths=[]` shorthand rather than a literal full-scope replay list.
