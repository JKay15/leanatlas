...
- if no stable closeout alias exists yet, inspect the live maintainer LOOP session evidence under `artifacts/loop_runtime/by_key/**`
- if a stable closeout alias exists, resolve the settled maintainer state through `artifacts/loop_runtime/by_execplan/<stable_execplan_id>/MaintainerCloseoutRef.json`
- only after that, follow any run-key-specific maintainer LOOP / session evidence referenced by the alias under `artifacts/loop_runtime/by_key/**`
- when an ExecPlan has a stable closeout ref, prefer `artifacts/loop_runtime/by_execplan/<stable_execplan_id>/MaintainerCloseoutRef.json` over hand-maintained prose
...
- Recent LOOP hardening cluster:
  ...
  - `20260308_external_paper_ingress_scope_discoverability_v0.md`
