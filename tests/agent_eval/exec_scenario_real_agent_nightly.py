#!/usr/bin/env python3
"""Nightly real-agent smoke for Phase6 scenario runner.

Runs one real-agent scenario through `run_scenario --mode run` and verifies
...
def main() -> int:
    ...
    scenario_path = (
        ...
        / "agent_eval"
        / "scenarios"
        / "mentor_keywords_interleaving_v0"
        / "scenario.yaml"
    )
    if not scenario_path.exists():
        print(f"[agent-eval-real][FAIL] missing scenario: {scenario_path}")
        return 2

    with tempfile.TemporaryDirectory(prefix="leanatlas_scenario_real_") as td:
        ...
        eval_root = out_root / "mentor_keywords_interleaving_v0"
        stamps = sorted([p for p in eval_root.iterdir() if p.is_dir()]) if eval_root.exists() else []
        if not stamps:
            print("[agent-eval-real][FAIL] run_scenario produced no eval dirs")
            return 2
    ...
    print("[agent-eval-real][OK] scenario real-agent smoke completed")
    return 0
