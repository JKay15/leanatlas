...
REQUIRED_SNIPPETS = {
    ...
    "docs/contracts/LOOP_PYTHON_SDK_CONTRACT.md": [
        ...
        "REPO_FILE_BYTES",
        "PATH_SET",
        "helper-derived merged scope back into `build_pyramid_review_plan(...)` alongside the same `followup_partition_ids` MUST preserve the same provenance metadata and `strategy_fingerprint`",
