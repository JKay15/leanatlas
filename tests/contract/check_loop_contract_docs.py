...
REQUIRED_SNIPPETS = {
    ...
    "docs/contracts/LOOP_GRAPH_CONTRACT.md": [
        ...
        "root-issued exception artifact",
        "delegated node ids",
    ...
    "docs/contracts/LOOP_PYTHON_SDK_CONTRACT.md": [
        ...
        "`looplib` import surfaces MUST NOT eagerly require LeanAtlas-only worktree helpers",
        "`UNPARSEABLE_REVIEW_OUTPUT`",
    ...
    "docs/contracts/LOOP_WAVE_EXECUTION_CONTRACT.md": [
        ...
        "provider stdout banner lines such as `model: gpt-5.4`, `provider: openai`, and `reasoning effort: low` MUST be parsed into `observed_runtime_metadata`",
        "`allowed_runtime_reasoning_efforts`",
        "`REVIEWER_PROFILE_MISMATCH`",
        "`REVIEWER_RUNTIME_METADATA_MISSING`",
        "`observed_runtime_metadata`",
        ...
        "helper-authored default no-escalation bundles may also keep the integrated closeout at `LOW` under `LOW_PLUS_MEDIUM`",
        "explicit `review_tier_policy` provenance (`LOW_ONLY` or `LOW_PLUS_MEDIUM`)",
        "`STRICT / xhigh` remains exceptional",
        "parent-wave automation must materialize a deterministic batch plan/state/journal",
        "published as append-only evidence, not hidden chat context",
        "rematerialized context packs",
        ...
        "child waves MUST NOT be recorded as `PASSED` unless every returned `result_refs` entry and `closeout_ref` resolves to an existing repo-root artifact",
        "re-running `execute_batch_supervisor(...)` on the same `run_key` MUST requeue persisted `RUNNING` child waves back to `PENDING`",
        "`CHILD_WAVE_REQUEUED`",
        ...
        "`allowed_runtime_reasoning_efforts=[stage.agent_profile]`",
