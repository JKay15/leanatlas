...
def main() -> int:
    ...
    for s in (
        ...
        "REVIEWER_RUNTIME_METADATA_MISSING",
        "observed_runtime_metadata",
        "`execute_review_orchestration_bundle(...)` MUST pass `allowed_runtime_reasoning_efforts=[stage.agent_profile]` and `require_observed_runtime_metadata=True`",
        "build_review_prompt(...)",
        "build_controlled_review_prompt_experiment(...)",
        ...
        "Helper-authored default no-escalation bundles MAY also keep `final_integrated_closeout.review_tier = LOW` under `LOW_PLUS_MEDIUM`",
        "the allowed values are `LOW_ONLY` and `LOW_PLUS_MEDIUM`",
        "preserve the `LOW_PLUS_MEDIUM` provenance rather than relabeling the plan as `LOW_ONLY`",
        "Any LOW integrated closeout, regardless of provenance, MUST keep `deep_partition_followup.partition_ids=[]`",
        ...
        "the default same-mode `CONTEXT_REBUILD` retry budget is reserved for child waves that declare `supervision_policy.executor_reasoning_effort = xhigh`",
        "explicit `allow_context_rebuild_retries = 0` MUST disable same-mode `CONTEXT_REBUILD` retries",
        ...
        "supervisor guidance artifacts MUST preserve explicit known conclusions, non-goals, and reminder text",
        "rematerialized context packs MUST canonicalize repo-root refs to repo-relative form before hashing or deduping them",
        "parent-supervisor helpers MUST validate returned `result_refs` / `closeout_ref` artifacts before marking a child `PASSED`",
        "`materialize_batch_supervisor(...)` / `execute_batch_supervisor(...)` MUST resume interrupted runs by requeueing persisted `RUNNING` child waves on the same `run_key`",
        "`CHILD_WAVE_REQUEUED`",
        "`INTERRUPTED_RUNNING_STATE`",
        "batch-supervisor resume on the same `run_key` MUST preserve prior attempt counts",
        "`looplib`",
        ...
        "session-issued root exception artifact",
        "execution_path",
        "root supervisor kernel",
        "conversation-facing task agent",
        "direct/manual fallback is exceptional",
        "proper subset of delegated nodes",
    ...
    for helper_name, helper in (
        ...
        ("issue_root_supervisor_exception", issue_root_supervisor_exception),
        ("publish_supervisor_guidance_event", publish_supervisor_guidance_event),
    ):
